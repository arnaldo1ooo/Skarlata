...
public class AdministrarUsuariosSession {
	...
	public static ResultSet obtenertabla() {
		...
		try {
			...
			rs = s.executeQuery("SELECT usu_codigo, usu_nombre, usu_apellido, usu_cedula, usu_login, usu_pass, CASE WHEN usu_estado = 0 THEN 'INACTIVO' ELSE 'ACTIVO' END, pri_descri FROM usuario, privilegio WHERE usu_codigo != 1 AND usu_privilegio = pri_codigo ORDER BY usu_codigo");
